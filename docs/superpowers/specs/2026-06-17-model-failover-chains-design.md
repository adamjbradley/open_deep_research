...
- **`failover.py` (new)** — pure policy, no I/O. Owns:
  ...
  - `AvailabilityTracker` — a run-scoped record of models marked **down** (hard-failed). `mark_down(model)`, `is_down(model)`, `available_chain(chain) -> list[str]` (the chain minus down models, order preserved), and `failovers` (the recorded `FailoverRecord` list, via `record_failover(...)`).
  - **Run-scoping is a module-level registry keyed by the run's `thread_id`** (`new_run_tracker(key)`, `get_tracker(key)`, `discard_tracker(key)`), *not* a `ContextVar` alone. This is load-bearing: LangGraph runs **each node in its own copied context**, so a `ContextVar` set in the entry node would be invisible to later nodes — stickiness wouldn't cross stages and `persist_research` would read an empty tracker. A module dict is shared across all node contexts; keying it by `thread_id` (same for every node in a run, fresh per run because `new_run_tracker` overwrites) gives a tracker that survives the whole run including the researcher fan-out. A key-less `ContextVar` fallback is retained for unit tests / one-off calls.
...
`model_routing.py` and the graph nodes are otherwise unchanged. The graph keeps calling `configurable_model.with_config({...}).ainvoke(...)`; the config dict now also carries the chain + stage. `configurable_claude_model.ainvoke` resolves the run's `thread_id` from an explicit `run_key` in its config or, failing that, from the ambient LangGraph config via `ensure_config()` (which LangGraph populates per node) — so no per-call-site `run_key` threading is needed and helper call-sites without a `config` parameter are covered. `preallocate_run` calls `new_run_tracker(thread_id)`; `persist_research` reads `get_tracker(thread_id).failovers` then `discard_tracker(thread_id)`.
...
- **Parallel fan-out + shared tracker:** ~~Should the `AvailabilityTracker` be shared across the researcher `asyncio.gather` tasks or per-task?~~ **RESOLVED in implementation:** one tracker per run, shared across the whole run (entry → all stages → fan-out → persist) via the `thread_id`-keyed registry. The fan-out children share it by reference; asyncio's single thread makes the `set.add`/`list.append` mutations race-safe. Known bounded limitation: a run that errors before `persist_research` leaves its registry entry undiscarded until a re-run with the same `thread_id` overwrites it (bounded by distinct thread_ids; a `finally`/reaper is a possible future hardening). A run with no `thread_id` (no checkpointer) silently degrades to the per-node `ContextVar` fallback — the real `deep_researcher` graph always has one.
